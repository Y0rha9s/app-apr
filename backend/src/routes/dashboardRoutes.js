...
// Helper para calcular el intervalo según rango
const getIntervalo = (rango) => {
  const rangos = {
    '7d':  '7 days',
    '15d': '15 days',
    '30d': '30 days',
  ...
  return rangos[rango] || '3 months';
};

// KPIs Principales
router.get('/kpis', async (req, res) => {
  try {
    const intervalo = getIntervalo(req.query.rango);

    const usuariosTotal = await pool.query(
      `SELECT COUNT(*) as total FROM usuarios WHERE rol = $1`, ['usuario']
    );
    const usuariosActivos = await pool.query(
      `SELECT COUNT(*) as total FROM usuarios WHERE rol = $1 AND estado = $2`, ['usuario', 'activo']
    );
    ...
    const consumoPromedio = await pool.query(
      `SELECT AVG(consumo_m3) as promedio
       FROM lecturas
       WHERE fecha_lectura >= CURRENT_DATE - INTERVAL '${intervalo}'`
    );
    const ingresosMes = await pool.query(
      `SELECT COALESCE(SUM(monto), 0) as total
       FROM pagos
       WHERE fecha_pago >= CURRENT_DATE - INTERVAL '${intervalo}'`
    );
...
router.get('/top-deudores', async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT 
        u.id, u.nombre, u.numero_cliente, u.rut,
  ...
  try {
    ...
    const nuevosMorosos = await pool.query(
      `SELECT u.nombre, u.numero_cliente, b.periodo, b.saldo_pendiente, b.fecha_vencimiento
       ...
       WHERE b.estado IN ('pendiente', 'parcial')
         AND b.saldo_pendiente > 0
         AND b.fecha_vencimiento >= CURRENT_DATE - INTERVAL '${intervalo}'
         AND b.fecha_vencimiento < CURRENT_DATE
    ...
    const consumoAnormal = await pool.query(
      `WITH promedios AS (
        SELECT usuario_id, AVG(consumo_m3) as promedio
        FROM lecturas
        WHERE fecha_lectura >= CURRENT_DATE - INTERVAL '${intervalo}'
