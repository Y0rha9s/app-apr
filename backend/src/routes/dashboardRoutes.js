...
router.get('/kpis', async (req, res) => {
    try {
        ...
        const usuariosActivos = await pool.query(
            'SELECT COUNT(*) as total FROM usuarios WHERE rol = $1 AND estado = $2',
            ['usuario', 'activo']
        );
       ...
       WHERE estado IN ('pendiente', 'parcial')
        ...
        // Consumo promedio mensual (últimos 3 meses)
        const consumoPromedio = await pool.query(
            `SELECT AVG(consumo_m3) as promedio
       FROM lecturas
       WHERE fecha_lectura >= CURRENT_DATE - INTERVAL '3 months'`
        );
...
router.get('/top-consumidores', async (req, res) => {
    try {
       ...
       WHERE l.fecha_lectura >= CURRENT_DATE - INTERVAL '3 months'
       ...
       LIMIT 10`
        ...
        console.error('Error obteniendo top consumidores:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});
...
router.get('/evolucion-consumo', async (req, res) => {
    try {
       ...
       FROM lecturas
       WHERE fecha_lectura >= CURRENT_DATE - INTERVAL '6 months'
       GROUP BY DATE_TRUNC('month', fecha_lectura)
       ORDER BY DATE_TRUNC('month', fecha_lectura) ASC`
        );
...
router.get('/alertas', async (req, res) => {
    try {
       ...
       WHERE b.estado IN ('pendiente', 'parcial')
         AND b.saldo_pendiente > 0
         AND b.fecha_vencimiento >= CURRENT_DATE - INTERVAL '7 days'
         AND b.fecha_vencimiento < CURRENT_DATE
       ORDER BY b.fecha_vencimiento DESC
       LIMIT 5`
        );

        // Consumo anormal (>30% del promedio del usuario)
        const consumoAnormal = await pool.query(
            `WITH promedios AS (
            SELECT 
                usuario_id,
                AVG(consumo_m3) as promedio
                FROM lecturas
                WHERE fecha_lectura >= CURRENT_DATE - INTERVAL '6 months'
                GROUP BY usuario_id
                HAVING AVG(consumo_m3) > 0
            ),
            ultimas_lecturas AS (
                ...
                fecha_lectura
                FROM lecturas
                ORDER BY usuario_id, fecha_lectura DESC
            )
            SELECT 
                u.nombre,
                u.numero_cliente,
                ul.consumo_m3 as consumo_actual,
                p.promedio as consumo_promedio,
            ...
            WHERE p.promedio > 0
                AND ABS((ul.consumo_m3 - p.promedio) / NULLIF(p.promedio, 0) * 100) > 30
            ORDER BY ABS((ul.consumo_m3 - p.promedio) / NULLIF(p.promedio, 0) * 100) DESC
            LIMIT 5`
        );

        // Cortes próximos (próximos 7 días)
        ...
        WHERE hc.estado = 'cortado'
          AND hc.fecha_corte <= CURRENT_DATE + INTERVAL '7 days'
